/*********************************************************************************************************************************************************
Program: Analysis_2nd_variant_VE_macro
...
******************************************************;
* Macro for test-negative design (TND) baseline table ;
******************************************************;

%macro TNDbaselinetable(indata);
...
* Get means, frequencies and p-values;
    %stat_TABLE(DSN=table, ID=mrn, by=outcome,
                var=vac_status age age_grp sex_n Race_eth BMI_grp smoking Charlson_wt_new /*<-new, continuous verson*/ Charlson_wt, 
                type=2 1 2 2 2 2 2 1 2, pvalues=Y, 
...
** Combine and reorder the data to be consistent with the table contents;
	* Create a total line at beginning;
    data Part1; set Part1;
    if _N_=1 then do; order=1; level='Total'; c1=cats('N=',byn1); c2=cats('N=',byn2); ctotal=cats('N=',nobs); pvalue=''; end;
    ...
    * Remove useless title lines;
    if _line=0 and level not in ('Comorbidities', 'Total') then delete; 
	...
	* Change p-value to N/A for matching vars;
...
* Get absolute standardized difference;
    ...
    %stddiff(inds =table, groupvar = outcome, 
             numvars  = age Frailty_index charlson_wt_new,
             charvars = age_grp sex_n Race_eth BMI_grp smoking Charlson_wt Frailty
                        ...
                        VA_grp ED_grp IP_grp preventive_care Medicaid h_med_income KPSC_employee medical_center_and_area index_month Specimen_Type_n vac_status /*<-new*/,
             stdfmt = 8.2, outds = table_std); 

* Merge to get all information for Table 1;
    proc sql; create table temp as 
    select a.order, a.var, a.positive as pos, a.negative as neg, a.pvalue as p, 
    ...
    data Table3; set temp;
    length Rowname $100. temp $100.;
    retain temp;
    if p^='' then temp=var;
    if p^='' or _N_=1 then Rowname=var;
    else Rowname=catt(temp,var);
	* Keep only Yes level for binary vars;
	level_laglag=lag(lag(Rowname));
	pvalue_laglag=lag(lag(p));
	ABS_laglag=lag(lag(ABS));
	if Rowname in ('IC','IC    0',
		'autoimmune','autoimmune    0',
		'pregnancy','pregnancy    0',
		'hist_covid_diag','hist_covid_diag    0',
		'hist_covid_test','hist_covid_test    0',
		'preventive_care','preventive_care    0',
		'medicaid', 'medicaid    0',
		'KPSC_employee', 'KPSC_employee    0') then delete;
	if Rowname in ('IC    1','autoimmune    1','pregnancy    1',
		'hist_covid_diag    1','hist_covid_test    1','preventive_care    1','medicaid    1','KPSC_employee    1') then do;
		var=level_laglag; p=pvalue_laglag; ABS=ABS_laglag; 
		end;
	if find(rowname, 'medical_center_and_area') and rowname^='medical_center_and_area' then delete; *Keep only subheader row for med center;
	drop level_laglag pvalue_laglag ABS_laglag order rowname temp;
    run;
...
%macro TND(indata, exposed_grp, comparison_grp, suffix, outcome, variant, variant_var=variant_subgroup, condition_model=1, time_since_vac=0);
	title "&suffix analysis, &variant";
    proc univariate data=&indata;
	where vac_status in ("&exposed_grp", "&comparison_grp") and &variant_var="&variant";
    var Frailty_index;
    ods output Quantiles=Qs;
    run;
    proc sql; 
    ...
    quit;
    data analysis; 
	length vac_status $20;
	set &indata;
	where vac_status in ("&exposed_grp", "&comparison_grp") and &variant_var="&variant";
    * Create frailty category based on top quartile;
        if Frailty_index >=&Q3 then Frailty=4;
        else if Frailty_index >=&Q2 then Frailty=3;
        else if Frailty_index >=&Q1 then Frailty=2;
        else Frailty=1;
    ...
    * Define variable used for ASD macro, so it can be called for model adjustment based on ASD value;
        sex_n=(sex='Male');
        Specimen_Type_n=(Specimen_Type='Saliva');
	* Define time interval (combine 91-150d and >150d for 4-dose comparison);
	%if "&exposed_grp"="4-dose" %then %do;
	if time_interval>90 then days_interval=">90 days";
	%end;	
	%if &time_since_vac=1 %then %do;
	if vac_status="&exposed_grp" then vac_status=catx(' ',vac_status,days_interval);
	%end;
    run;
    * Get frequency counts;
    proc freq data=analysis;
    table &outcome.*vac_status/nocol nopercent or;
    ods output CrossTabFreqs=counts;
    run;
    proc sql;
	create table count_pct as
	select a.*, b.frequency as frequency_unexp,
	cat(a.frequency, ' (', compress(put((a.frequency*100/(a.frequency+b.frequency)), 5.1),' '), '%)') as counts_exp,
	cat(b.frequency, ' (', compress(put((b.frequency*100/(a.frequency+b.frequency)), 5.1),' '), '%)') as counts_unexp
	from counts(where=(_type_='11' and find(vac_status, "&exposed_grp"))) a
	inner join counts(where=(_type_='11' and vac_status="&comparison_grp")) b
	on a.&outcome = b.&outcome;
	quit;
	* Get unadjusted OR;
    proc logistic data=analysis desc;
    class vac_status(ref="&comparison_grp")/param=reference;
    model &outcome.=vac_status;
    ...
    * Get adjusted OR;
    proc logistic data=analysis desc;
    class vac_status(ref="&comparison_grp") age_grp sex Race_eth(ref="Hispanic") &vars_class /param=reference;
    model &outcome.=vac_status &vars_class &vars_num %if &condition_model=0 %then %do; age_grp sex Race_eth %end; / maxiter=100;
    %if &condition_model=1 %then %do; strata pair_id; %end;
    ...
    if UpperCL>1 then VELowerCL=round((1/UpperCL-1)*100,0.1);
    run;
    * Put all results together - main analyses;
	%if &time_since_vac=0 %then %do;
    proc sql; create table results_&variant as
    select "&variant" as Variant length=25,
           a.counts_exp as P_exp, a.counts_unexp as P_unexp, '' as separator1, 
           b.counts_exp as N_exp, b.counts_unexp as N_unexp, '' as separator2,
           cat(put(c.OddsRatioEst, 5.3), ' (', put(c.LowerCL, 5.3), ', ', put(c.UpperCL,5.3), ')') as OR,
           cat(put(d.OddsRatioEst, 5.3), ' (', put(d.LowerCL, 5.3), ', ', put(d.UpperCL,5.3), ')') as adjOR, 
           '' as separator3,
           cat(strip(put(c.VEest,5.1)), '% (', strip(put(c.VELowerCL,5.1)), '%, ', strip(put(c.VEUpperCL,5.1)), '%)') as VE,
           cat(strip(put(d.VEest,5.1)), '% (', strip(put(d.VELowerCL,5.1)), '%, ', strip(put(d.VEUpperCL,5.1)), '%)') as adjVE,
           '' as separator4,
           d.VEest as estimate, 
           d.VELowerCL as lower_CI, 
           d.VEUpperCL as upper_CI
		from count_pct(where=(&outcome=1)) a,   
           count_pct(where=(&outcome=0)) b, 
           OR(where=(substr(effect,1,10)='vac_status')) c, 
           AdjOR(where=(substr(effect,1,10)='vac_status')) d, 
           PE(where=(variable='vac_status')) e; 
    quit;
	%end;
    * Put all results together - time since vac analyses;
	%if &time_since_vac=1 %then %do;
    proc sql; create table results_time_since_vax_&variant as
    select catx(' ',"&variant",a.vac_status) as Variant length=25,
           a.counts_exp as P_exp, a.counts_unexp as P_unexp, '' as separator1, 
           b.counts_exp as N_exp, b.counts_unexp as N_unexp, '' as separator2,
           cat(put(c.OddsRatioEst, 5.3), ' (', put(c.LowerCL, 5.3), ', ', put(c.UpperCL,5.3), ')') as OR,
           cat(put(d.OddsRatioEst, 5.3), ' (', put(d.LowerCL, 5.3), ', ', put(d.UpperCL,5.3), ')') as adjOR, 
           ...
           d.VEUpperCL as upper_CI
      from count_pct(where=(&outcome=1)) a   
           left join count_pct(where=(&outcome=0)) b on a.vac_status=b.vac_status
		   left join OR(where=(substr(effect,1,10)='vac_status')) c on find(c.effect, trim(a.vac_status)) 
           left join AdjOR(where=(substr(effect,1,10)='vac_status')) d on find(d.effect, trim(a.vac_status)) 
           left join PE(where=(variable='vac_status')) e on a.vac_status=e.classval0; 
    quit;
	%end;
	title;
%mend TND;

* Define macro to combine analysis results;
%macro TND_combine(output); 
    data &output; 
    length variant $25.;
    set results:;
    drop separator4 estimate lower_ci upper_ci;
    run;
	proc sort data=&output; by variant; run;
    data &output._plotdata;
    length variant $25. counts $500.;
    set results:;
    counts=cat(strip(P_exp),'/', strip(N_exp));
    keep variant counts estimate lower_ci upper_ci;
    run;
	proc sort data=&output._plotdata; by variant; run;
	title "&output";
